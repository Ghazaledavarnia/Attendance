...
var kamaDatepicker = function (elementID, opt) {
    ...
    titleDiv.append("<button id='bd-next-" + elementID + "' class='bd-next' type='button'  data-toggle='tooltip'><span>بعدی</span></button>");
    ...
    titleDiv.append("<button id='bd-prev-" + elementID + "' class='bd-prev' type='button'  data-toggle='tooltip'><span>قبلی</span></button>");
    ...
    inputElement.on("focus click", function () {
        ...
        if (goToNext) {
            data[1] = data[1].length === 1 ? "0" + data[1].toString() : data[1].toString();
            data[2] = data[2].length === 1 ? "0" + data[2].toString() : data[2].toString();
            $(this).val(data.join('/'))
        }
    ...
    var makeYearList = function (thisYear) {
        ...
        for (i = 0; i < 50; i++) {
            ...
            var tempYear = ((baseYear) + i) + '';//thisYear - 10
    ...
    inputElement.parent().on("click", "button.day", function () {
        ...
        selectedMonth = selectedMonth < 10 ? '0' + selectedMonth.toString() : selectedMonth.toString();
        selectedDay = $(this).attr('class').split(" ")[$(this).attr('class').split(" ").indexOf('day') + 1].split("-")[1];
        selectedDay = selectedDay.length === 1 ? '0' + selectedDay.toString() : selectedDay.toString();
        var datestr = selectedYear + "/" + selectedMonth + "/" + selectedDay;
        if (options.twodigit) {
